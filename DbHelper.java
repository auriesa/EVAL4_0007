...
public class DbHelper extends SQLiteOpenHelper {
    ...
    public void onCreate (SQLiteDatabase sqLiteDatabase) {
        ...
        sqLiteDatabase.exeSQL(sql);
    }
    @Override
    public void onUpgrade(SQLiteDatabase sqLiteDatabase, int i, int i1) {
    }
    public void insertData (SQLiteDatabase db, Mahasiswa db){
    ...
    public ArrayList<Mahasiswa> getArrayListMhs(SQLiteDatabase db) {
        ...
        if (cursor.getCount()> 0) {
            cursor.moveToFirst();
            do {
                arrayListMhs.add(new Mahasiswa(cursor.getString(0), cursor.getString(1), cursor.getString(2)));
